...
;(function()
  ...
  local level9Exp = Mon.experienceForLevel(growth, 9)
  local level10Exp = Mon.experienceForLevel(growth, 10)
  for _, mode in ipairs({ "hard", "hardcore" }) do
    run.loader.modSave["crystal_legacy_changes"] = { difficulty = mode }
    local capped = Mon.new(runtimeData, "PIKACHU", 9, { moves = {} })
    capped.experience = level9Exp
    ...
    T.eq(gained, level10Exp - level9Exp,
      mode .. " clamps battle EXP at the level cap")

    local candyMon = Mon.new(runtimeData, "PIKACHU", 10, { moves = {} })
    local candy = ItemEffects.useOnMon("RARE_CANDY", candyMon, runtimeData)
    T.check(not candy.used and candyMon.level == 10,
